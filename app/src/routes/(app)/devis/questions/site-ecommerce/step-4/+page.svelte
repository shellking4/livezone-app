<script lang="ts">
    ...
    const handleNext = () => {
        let naturalSeoInquiry = (document.querySelector('input[name="naturalseo"]:checked') as HTMLInputElement).value;
        let estimateRecap = $estimateData
        let data = {
            question: questionNumber,
            outline: `Optimisation au référencement naturel`,
            answer: `${naturalSeoInquiry}`,
            price: (function () {
                let price = 0;
                for (let i = 0; i < priceDetails.length; i++) {
                    if ((priceDetails[i][0] as string).toLowerCase().includes(naturalSeoInquiry)) {
        ...
        goto("/devis/questions/site-ecommerce/step-5")
    }
</script>
<div class="bg-gray-200">
    ...
    <div class="px-4 lg:w-full max-w-screen-xl lg:mx-auto mx-3 mt-10 lg:py-4 py-4 bg-white border border-white rounded-3xl shadow sm:p-16 dark:bg-gray-800 dark:border-gray-700">
        <img
            ...
            alt=""
        />
        <p class="mb-5 text-lg text-gray-900 font-normal mt-4 sm:text-lg dark:text-black-900">QUESTION 4 SUR 5</p>
        <div class="divide-y-[12px] divide-slate-400/[.24]">
            <div></div><div></div>
        </div>
        <h5 class="mb-5 mt-[15px] lg:text-2xl text-md font-bold lg:text-left text-center text-gray-900 dark:text-white">Optimisation au référencement naturel</h5>
        <p class="mb-5 lg:text-lg text-sm text-gray-900 font-normal mt-4 sm:text-lg dark:text-black-900">C'est le processus d'optimisation des sites web pour les moteurs de recherche.</p>
        ...
        <div class="flex items-center pl-4 py-1 bg-gray-100 border border-gray-300 rounded-3xl dark:border-gray-700">
            <input checked={true} id="yes" type="radio" value="oui" name="naturalseo" class="lg:w-7 lg:h-7 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            <label for="bordered-radio-1" class="w-full lg:py-4 py-2 lg:px-7 px-1 ml-2 lg:text-2xl text-md font-normal text-gray-900 dark:text-gray-300">Oui</label>
        </div>

        <div class="mt-5"></div>

        <div class="flex items-center pl-4 py-1 bg-gray-100 border border-gray-300 rounded-3xl dark:border-gray-700">
            <input id="no" type="radio" value="no" name="naturalseo" class="lg:w-7 lg:h-7 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            <label for="bordered-radio-1" class="w-full lg:py-4 py-2 lg:px-7 px-1 ml-2 lg:text-2xl text-md font-normal text-gray-900 dark:text-gray-300">Non</label>
        </div>
